...
<?= $this->section('title') ?>Edit Aturan Bonus<?= $this->endSection() ?>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Edit Aturan Bonus</h3>
                </div>
                <div class="card-body">
                    ...
                    <form action="<?= base_url('admin/bonus/rules/update/' . $rule['bonus_id']) ?>" method="post" id="bonusForm">
                        <?= csrf_field() ?>
                        
                        <div class="mb-3">
                            <label for="rule_name" class="form-label">Nama Aturan Promo <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="rule_name" name="rule_name" value="<?= old('rule_name', $rule['rule_name']) ?>" required>
                        </div>

                        <div class="mb-3">
                            <label for="bonus_item_name" class="form-label">Nama Item Bonus / Hadiah <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="bonus_item_name" name="bonus_item_name" value="<?= old('bonus_item_name', $rule['bonus_item_name']) ?>" required>
                        </div>
                                ...
                                <table class="table table-sm table-bordered table-striped table-hover mb-0" id="productChecklistTable">
                                    ...
                                    <tbody>
                                        <?php if (!empty($products)): ?>
                                            <?php 
                                                $rawString = trim($rule['applicable_product_ids'] ?? '');
                                                $rawSavedIds = !empty($rawString) ? explode(',', $rawString) : [];
                                                $savedIds = array_map('trim', $rawSavedIds);
                                            ?>
                                            <?php foreach ($products as $product): ?>
                                                <?php 
                                                    // PERBAIKAN UTAMA: Membaca kolom product_id (string PRDK001) agar sinkron dengan database
                                                    ...
                                                    $currentId = trim((string)$pId);
                                                    $isChecked = ($currentId !== '') && in_array($currentId, $savedIds, true);
                                                ?>
                                                <tr class="product-row" data-category="<?= $pCatId ?>" style="cursor: pointer;">
                                                    <td class="text-center">
                                                        <input class="form-check-input prod-checkbox" type="checkbox" name="product_ids[]" value="<?= $currentId ?>" <?= $isChecked ? 'checked' : '' ?>> 
                                                    </td>
                                                    <td class="prod-name">
                                                        <?= esc($pNama) ?>
                                                    </td>
                                        ...
                                        <div class="row g-2 align-items-center mb-2 tier-row">
                                            <div class="col-md-5">
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <input type="number" class="form-control" name="min_price[]" value="<?= $minPrice ?>" required>
                                                </div>
                                            </div>
                                            <div class="col-md-5">
                                                <div class="input-group input-group-sm">
                                                    <span class="input-group-text">Dapat Hadiah:</span>
                                                    <input type="number" class="form-control" name="bonus_amount[]" value="<?= $amount ?>" required>
                                                    <span class="input-group-text">Pcs</span>
                                                </div>
                                            </div>
                                            <div class="col-md-2">
                                                <button type="button" class="btn btn-sm btn-danger w-100 remove-tier-btn" <?= $isFirst ? 'disabled' : '' ?>>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label for="quota_limit" class="form-label">Kuota Total</label>
                                    <input type="number" class="form-control" id="quota_limit" name="quota_limit" value="<?= old('quota_limit', $rule['quota_limit']) ?>" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label for="usage_count" class="form-label">Kuota Terpakai</label>
                                    <input type="number" class="form-control" id="usage_count" name="usage_count" value="<?= old('usage_count', $rule['usage_count']) ?>" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control" id="is_active" name="is_active" required>
                                        <option value="1" <?= old('is_active', $rule['is_active']) == '1' ? 'selected' : '' ?>>Aktif</option>
                                        <option value="0" <?= old('is_active', $rule['is_active']) == '0' ? 'selected' : '' ?>>Non-Aktif</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="datetime-local" class="form-control" id="start_date" name="start_date" value="<?= date('Y-m-d\TH:i', strtotime($rule['start_date'])) ?>" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="end_date" class="form-label">Tanggal Berakhir</label>
                                    <input type="datetime-local" class="form-control" id="end_date" name="end_date" value="<?= date('Y-m-d\TH:i', strtotime($rule['end_date'])) ?>" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> Perbarui Aturan Bonus
                            </button>
                        </div>
                    </form>
                </div>
