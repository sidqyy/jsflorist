...
<div class="row">
    <div class="col-md-8">
        <div class="card">
            ...
            <form action="<?= base_url('admin/event-banners/update/' . $eventBanner['id']) ?>" method="post" enctype="multipart/form-data">
                <?= csrf_field() ?>
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="title" name="title"
                            value="<?= old('title', $eventBanner['title']) ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="image">Gambar Event Banner</label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="url" class="form-control" id="link_url" name="link_url"
                            value="<?= old('link_url', $eventBanner['link_url']) ?>" placeholder="https://example.com">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="datetime-local" class="form-control" id="start_date" name="start_date"
                                    value="<?= old('start_date', date('Y-m-d\TH:i', strtotime($eventBanner['start_date']))) ?>" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="datetime-local" class="form-control" id="end_date" name="end_date"
                                    value="<?= old('end_date', date('Y-m-d\TH:i', strtotime($eventBanner['end_date']))) ?>" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="is_active" name="is_active">
                            <option value="1" <?= old('is_active', $eventBanner['is_active']) == '1' ? 'selected' : '' ?>>Aktif</option>
                            <option value="0" <?= old('is_active', $eventBanner['is_active']) == '0' ? 'selected' : '' ?>>Tidak Aktif</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="domain_specific" name="domain_specific" onchange="toggleDomainSelection()">
                            <option value="0" <?= old('domain_specific', $eventBanner['domain_specific'] ?? 0) == '0' ? 'selected' : '' ?>>Tampilkan di Semua Domain</option>
                            <option value="1" <?= old('domain_specific', $eventBanner['domain_specific'] ?? 0) == '1' ? 'selected' : '' ?>>Domain Spesifik</option>
                        </select>
                    </div>

                    <?php
                    $allowedDomains = [];
                    if (!empty($eventBanner['allowed_domains'])) {
                        $allowedDomains = json_decode($eventBanner['allowed_domains'], true) ?: [];
                    }
                    ?>

                    <div class="form-group" id="domain_selection" style="display: <?= ($eventBanner['domain_specific'] ?? 0) == 1 ? 'block' : 'none' ?>;">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" value="jsflorist.com" id="domain_js" name="allowed_domains[]"
                                <?= in_array('jsflorist.com', array_merge($allowedDomains, old('allowed_domains', []))) ? 'checked' : '' ?>>
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" value="poppyflorist.com" id="domain_poppy" name="allowed_domains[]"
                                <?= in_array('poppyflorist.com', array_merge($allowedDomains, old('allowed_domains', []))) ? 'checked' : '' ?>>
    ...
    <div class="col-md-4">
        <div class="card">
            ...
            <div class="card-body text-center">
                <img id="imagePreview"
                    src="<?= base_url('uploads/event_banners/' . $eventBanner['image_url']) ?>"
                    alt="Preview"
                    class="img-fluid"
